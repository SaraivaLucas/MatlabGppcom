...
%% Parâmetros
T=0.002;                                                % Taxa de amostragem 
Tf=1;                                                   % Tempo final em segundos
t=0:T:1;                                                % Definição do eixo do tempo      
fm1=3;                                                  % Frequência senoide 1      
fm1=1;                                                  % Frequência senoide 2
m_t=sin(2*pi*t)-sin(6*pi*t);                            % Sinal mensagem m(t)
lfft=length(m_t);                                       % Comprimento da fft
ts=0.02;                                                % Nova taxa de amostragem
Nfactor=ts/T;                                           % Número de elementos 
% send the signal hrough a 16-level uniform quantizer
[s_out,sq_out,sqh_out,Delta,SQNR]=sampandquant(m_t,16,T,ts);
% receive 3 signals:
...
%  domain.
figure(1);
subplot(311); sfigla=plot(t,m_t,'k');
hold on; sfig1b=plot(t,s_out(1:lfft),'b');hold off;
...
s_recv=s_recv(1:lfft); % reconstructed t-domian
...
subplot(212); sfig2b=plot(t,m_t,'k-.',t,s_recv(1:lfft),'b');
...
s_recv2=s_recv2(1:lfft);               % reconstructed t-domain

% plot the ideally reconstructed signal in time and frequency domain
figure(3)
subplot(211); sfig3a=plot(t,m_t,'b',t,s_ni(1:lfft),'b');
xlabel('time (sec)');
title('original signal versus flat-top reconstruction');
subplot(212);sfig3b=plot(t,m_t,'b',t,s_recv2(1:lfft),'b--');
